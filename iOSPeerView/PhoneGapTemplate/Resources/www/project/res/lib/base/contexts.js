...
var SimpleSecurityContext = createClass(SecurityContext, {
	...
	isLoggedIn: function() {
		return !!getVariable(this.__getLoginStorageKeyName());
	},
	...
	login: function(params) {
		...
		setVariable(this.__getLoginStorageKeyName(), login);
		setVariable(this.__getPasswordStorageKeyName(), password);
	},
	
	logout: function() {
		setVariable(this.__getLoginStorageKeyName(), '');
		setVariable(this.__getPasswordStorageKeyName(), '');
	},
	
	invoke: function(service, settings) {
		var login = getVariable(this.__getLoginStorageKeyName());
		var password = getVariable(this.__getPasswordStorageKeyName());
...
var OAuthSecurityContext = createClass(SecurityContext, {
	...
	getToken: function() {
		var token = getVariable(this.__getTokenStorageKeyName());
		var tokenSecret = getVariable(this.__getTokenSecretStorageKeyName());
	...
	__saveToken: function(token) {
		setVariable(this.__getTokenStorageKeyName(), token.token);
		setVariable(this.__getTokenSecretStorageKeyName(), token.tokenSecret);
	},
...
var OAuth2UsernamePasswordSecurityContext = createClass(SecurityContext, {
	...
	__saveToken: function(token) {
		setVariable(this.__getAccessTokenStorageKeyName(), token.accessToken);
		setVariable(this.__getTokenInstanceUrlStorageKeyName(), token.instanceUrl);
	},
	...
	getToken: function() {
		var accessToken = getVariable(this.__getAccessTokenStorageKeyName());
		var instanceUrl = getVariable(this.__getTokenInstanceUrlStorageKeyName());
